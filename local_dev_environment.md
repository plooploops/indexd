...
For Azure, please see the corresponding [Azure notes](./azure_environment.md).
...
You can also view the local [Postgres init.sql](./deployment/scripts/postgresql/postgres_init.sql) and run these inside local Postgres server.
...
Prior to installing requirements, be sure that [test-requirements.txt](test-requirements.txt) includes:
...
python3 -m pytest -vv --cov=indexd --cov-report xml tests
```
...
You may also need to update the [test settings](./tests/default_test_settings.py) with appropriate database connection information prior to running the tests.

```python
settings["config"]["TEST_DB"] = "postgres://username:password@localhost/test_migration_db"
```
